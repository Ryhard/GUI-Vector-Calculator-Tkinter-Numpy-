...
# Listas para los vectores y resultados
listaV1 = []
listaV2 = []
resultado = []
...
# Función para mostrar los vectores
def mostrarGraficaVectores():
    ...
    try:
        ...
        # Si está en 3D, obtener el tercer valor
    ...
    # Obtener los ángulos de los vectores
    try:
    ...
    # Graficar Vector Resultado si listaResultado contiene valores
    if resultado:
        vecResultado_x = resultado[0]
        vecResultado_y = resultado[1]
        angResultado = float(entryAngA.get())  # Suponiendo que se quiera rotar el resultado con el mismo ángulo que A
...
# Nueva función para mostrar gráfica en 3D
def mostrarGraficaVectores3D():
    ...
    fig = plt.figure(figsize=(6, 5))  # Definimos el tamaño de la figura desde el inicio
    fig.patch.set_facecolor('lightblue')  # Color de fondo de la figura
    ax = fig.add_subplot(111, projection='3d')
    ax.set_facecolor('lightblue')  # Color de fondo del área de dibujo del gráfico 3D
...
# Widgets de la GUI (mueve los widgets dentro del frame)
...
# Posicionamiento de widgets dentro del frame
